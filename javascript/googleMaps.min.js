...
typeof l.address&&0<l.address.length&&GoogleMapConstructors[g].googleMap.findAddress(l.address);0===g&&0<jQuery("#SearchByAddressForm_SearchByAddressForm").length&&jQuery("#SearchByAddressForm_SearchByAddressForm").ajaxForm({error:function(){alert("Could not find address!")},success:function(a,b,c,d){"undefined"!==typeof a.ErrorMessage?alert(a.ErrorMessage):(b=jQuery("base").attr("href")+"/googlemap/",b+=""+a.Action+"/",b+=""+a.ParentID+"/",b+=""+a.Title+"/",b+=""+a.Lng+"/",b+=""+a.Lat+"/",b+=""+
a.ClassNames+"/",GoogleMapConstructors[0].googleMap.addLayer(b,a.Title))},dataType:"json"})}});
...
a.variableName+".lastMarker,'clickRemoveMe'); return false;\">"+this.opts.addDeleteMarkerButton+"</a>");h.length&&(c+='<p class="infoTabExtraLinks">'+h.join(", ")+".</p>");google.maps.event.addListener(b,"clickZoomIn",function(){(new google.maps.MaxZoomService).getMaxZoomAtLatLng(b.position,function(c){c.status!=google.maps.MaxZoomStatus.OK?alert(a._t.error_in_zoom):(a.mapObject.setZoom(c.zoom),a.mapObject.setCenter(b.position))})});google.maps.event.addListener(b,"clickRemoveMe",function(){b.setMap(null)});
...
this.latestUrl=b,downloadUrl(b,function(b){a.processXml(b)}))},processXml:function(b){this.bounds=new google.maps.LatLngBounds;if(0<typeof b.getElementsByTagName("ErrorMessage").length)alert(b.getElementsByTagName("ErrorMessage")[0]);else if(0<b.getElementsByTagName("pointcount").length){var c=b.getElementsByTagName("pointcount")[0].childNodes[0].nodeValue,c=parseInt(c);this.tooManyPointsWarning(c+1);var d=null;if(0<c){var d=this.layerInfo.length,e={},f=[];e.show=1;e.url=this.latestUrl;this.latestUrl=
...
this.updateStatus(a._t.one_location_loaded))}else{c=b.getElementsByTagName("title")[0];f=b.getElementsByTagName("errormessage");if(0<f.length)for(e=0;e<f.length;e++)m=b.getElementsByTagName("errormessage")[e].innerHTML,alert(m);this.opts.titleId&&(el=document.getElementById(this.opts.titleId),el.innerHTML=c);this.opts.changePageTitle&&document.title&&(document.title=c)}}},tooManyPointsWarning:function(b){100<b?this.updateStatus(b+" "+a._t.large_number_of_items):0===b&&this.updateStatus(a._t.no_location_could_be_found)},
...
b.length){for(var e='<ul id="'+this.opts.sideBarId+'list">',f=0;f<b.length;f++)var c=b[f].split("$$$",2),h=c[1],c=this.gmarkers[h].layerId,e=-1==String(this.gmarkers[h].serverId).indexOf("manuallyAdded",0)?e+('<li class="forLayer'+c+" icon"+h+'"> <a href="'+this.currentPageURL+'#GoogleMapDiv" onclick="'+a.variableName+".showMarkerFromList("+h+'); return false;">'+this.gmarkers[h].markerName+'</a> <div class="infowindowDetails">'+this.gmarkers[h].markerDesc+"</div></li>"):e+('<li class="forLayer'+
c+'">'+a._t.you_added+': <a href="'+this.currentPageURL+'#GoogleMapDiv" onclick="'+a.variableName+".showMarkerFromList("+h+'); return false;">'+this.gmarkers[h].markerName+"</a></li>");d.innerHTML=e+"</ul>";"undefined"!==typeof AdjustHeightsForGoogleMap&&(AdjustHeightsForGoogleMap.boxesSelector="#"+this.opts.sideBarId+"list > li",jQuery(window).resize(function(){AdjustHeightsForGoogleMap.adjustBoxes()()}),AdjustHeightsForGoogleMap.adjustBoxes()())}}else console.debug("you defined the dropbox like this "+
...
b)},updateStatus:function(b,c){var d="",e="";"help"==c&&(b&&(b+="<hr />"),b+="<hr />"+this.helpHtml()+"");document.getElementById(this.mapDivName)&&(this.mapIsZoomed?(e="reduce map size",d=""):(e="full-screen",d='<span>|</span> <a href="#" onclick="'+a.variableName+'.hideStatus();">'+a._t.hide+"</a> "));d='<p class="helpLink" style="text-align: right; font-size: 10px; width: auto; float: right;">'+(' <a href="#" onclick="'+a.variableName+".updateStatus('', 'help'); return false;\"> "+a._t.show_help+
' </a> <span>|</span> <a href="#" onclick="'+a.variableName+'.enlargeMap(); return false;" id="mapZoomLinkLabel">'+e+"</a> "+d+"</p>"+b);this.opts.statusDivId&&(e=document.getElementById(this.opts.statusDivId),null!==e&&(e.innerHTML=d,e.style.display="block"))},hideStatus:function(){if(this.opts.statusDivId){var b=document.getElementById(this.opts.statusDivId);null===b||this.mapIsZoomed?this.mapIsZoomed&&alert(a._t.can_not_hide_bar_in_full_screen_mode):(b=document.getElementById(this.opts.statusDivId),
b.style.display="none")}},showAddress:function(b){var c="";this.opts.defaultCountryCode&&(c=this.opts.defaultCountryCode);a.geocoder=new google.maps.Geocoder;if(a.geocoder)return this.updateStatus(a._t.search_for_address),this.mapAddress=b,a.geocoder.geocode({address:b,region:c},function(b,c){if(c==google.maps.GeocoderStatus.OK){console.debug(b);var d=b[0];a.addAddressToMap(d);a.mapObject.setCenter(d.geometry.location)}else alert("Geocode was not successful for the following reason: "+c)}),!1;this.updateStatus(a._t.address_finder_not_loaded)},
addAddressToMap:function(b){place=b.geometry.location;var c=new google.maps.LatLng(place.lat(),place.lng());b=place.formatted_address;c=a.createPointXml(a._t.address_search+": "+a.mapAddress,c,b);a.processXml(c);a.updateStatus(a._t.address_found+": "+b);b=a.opts.updateServerUrlAddressSearchPoint+encodeURIComponent(b)+"/"+place.lng()+"/"+place.lat()+"/";a.addLayer(b);return!0},updateAddressFormFields:function(b){a.opts.latFormFieldId&&null!==document.getElementById(a.opts.latFormFieldId)&&(document.getElementById(a.opts.latFormFieldId).value=
b.lat(),document.getElementById(a.opts.lngFormFieldId).value=b.lng())},showRoute:function(){this.floatFrom?this.floatTo?(a.routeShown=!0,this.updateStatus(a._t.searching_for_route),a.directions.load(this.floatFrom+" to "+this.floatTo,{locale:this.opts.localeForResults,getSteps:!0})):this.updateStatus(a._t.no_valid_end):this.updateStatus(a._t.no_valid_start)},createRouteFromLayer:function(b){var c=[],d=0,e=!1;2>this.layerInfo[b].pointCount&&(e=!0);for(var f=0;f<this.gmarkers.length;f++)if(this.gmarkers[f].layerId==
...
b='<span id="currentlySetToFrom">'+this.currentlySetToString+this.from+"</span>":this.floatFrom&&(b='<span id="currentlySetToTo">'+this.currentlySetToString+this.floatFrom+"</span>");return b},currentToLocation:function(){var b="";this.to?b=this.currentlySetToString+this.to:this.floatTo&&(b=this.currentlySetToString+this.floatTo);return b},clearRouteVariables:function(){this.to="";this.floatTo=0;this.from="";this.floatFrom=0;var b=document.getElementById("currentlySetToFrom");null!==b&&(b.innerHTML=
"");b=document.getElementById("currentlySetToTo");null!==b&&(b.innerHTML="")},clearRouteAll:function(){var b=document.getElementById(a.opts.directionsDivId);null!==b&&(b.innerHTML="");this.clearRouteVariables();this.routeShown=!1;a.directions.clear();a.updateStatus("Route Cleared")},directionsHandleErrors:function(){a.directions.getStatus().code==G_GEO_UNKNOWN_ADDRESS?a.updateStatus("No corresponding geographic location could be found for one of the specified addresses. This may be due to the fact that the address is relatively new, or it may be incorrect.\nError code: "+
a.directions.getStatus().code):a.directions.getStatus().code==G_GEO_SERVER_ERROR?a.updateStatus("A geocoding or directions request could not be successfully processed, yet the exact reason for the failure is not known.\n Error code: "+a.directions.getStatus().code):a.directions.getStatus().code==G_GEO_MISSING_QUERY?a.updateStatus("The HTTP q parameter was either missing or had no value. For geocoder requests, this means that an empty address was specified as input. For directions requests, this means that no query was specified in the input.\n Error code: "+
a.directions.getStatus().code):a.directions.getStatus().code==G_GEO_BAD_KEY?a.updateStatus("The given key is either invalid or does not match the domain for which it was given. \n Error code: "+a.directions.getStatus().code):a.directions.getStatus().code==G_GEO_BAD_REQUEST?a.updateStatus("A directions request could not be successfully parsed.\n Error code: "+a.directions.getStatus().code):a.updateStatus("An unknown error occurred - Maybe the start or end address could not be found?")},directionsOnLoad:function(){function b(b,
c,d,f){e+='<tr> <th colspan="2">  <a href="#" onclick="'+a.variableName+".mapObject.showMapBlowup(new google.maps.LatLng("+b.toUrlValue(6)+')); return false;">   <img src="http://www.google.com/intl/en_ALL/mapfiles/icon-dd-'+c+'-trans.png" alt="marker">  </a>'+f+" ("+d+") </th></tr>"}function c(b,c,d,f){e+='<tr> <td>  <a href="#" onclick="'+a.variableName+".mapObject.showMapBlowup(new google.maps.LatLng("+b.toUrlValue(6)+')); return false;">'+c+".</a> "+d+' </td> <td class="dist">'+f+" </td></tr>"}
function d(b){var a="",a=99<b?a+((b/1E3).toFixed(1)+"&nbsp;km."):a+(b+"&nbsp;me.");miles=6.21371192E-4*b;return a+=" / "+(miles.toFixed(1)+" mi.")}a.updateStatus("Route Found - Details Loading . . .");for(var e="",f=null,h=null,g=null,k="",l="",n=0;n<a.directions.getNumRoutes();n++)for(0===n?(k="play",l=a.from):(k="pause",l=a.via),g=a.directions.getRoute(n),f=g.getStartGeocode(),h=g.getStep(0).getLatLng(),b(h,k,f.address,l),e+='<tr><th colspan="2" class="routeDistance">'+(d(g.getDistance().meters)+
" - about "+g.getDuration().html)+"</th></tr>",f=0;f<g.getNumSteps();f++)h=g.getStep(f),c(h.getLatLng(),f+1,h.getDescriptionHtml(),d(h.getDistance().meters));f=g.getEndGeocode();h=g.getEndLatLng();l=a.to;b(h,"stop",f.address,l);e+='<tr><th colspan="2" class="copyright">'+a.directions.getCopyrightsHtml()+"</th></tr>";e='<div id="directionsInnerDiv"> <table class="directionsClassTable"><col class="col1" /><col class="col2"/>'+e+"</table></div>";(g=document.getElementById(a.opts.directionsDivId))?(e+=
'<p id="directionsPrintOptions"><a href="#" onclick="'+a.variableName+'.printDirections(); return false;">Print Directions</a>, <a href="#" onclick="'+a.variableName+'.clearRouteAll(); return false;">Clear Route</a></p>',g.innerHTML=e,a.updateStatus('Directions Loaded - <a href="#" onclick="'+a.variableName+'.printDirections(); return false;on">Print Directions</a>.')):a.openDirectionsPopup(e);a.updateStatus("Route Loaded");a.clearRouteVariables();a.attachStyleDirections(document)},enlargeMap:function(){var b=
document.getElementById(this.mapDivName),c=document.getElementById("mapZoomLinkLabel");null!==b&&c&&(!0===this.mapIsZoomed?(b.style.width=this.mapOriginalSize.width+"px",b.style.height=this.mapOriginalSize.height+"px",b.style.position="relative",b.style.top="",b.style.left="",b.style.zIndex="",document.body.style.overflow="auto",this.mapIsZoomed=!1,c.innerHTML="full-screen"):(b.style.width=this.viewPortWidth()+"px",b.style.height=this.viewPortHeight()+"px",b.style.position="fixed",b.style.top=0,b.style.left=
0,b.style.zIndex="9",document.body.style.overflow="hidden",this.mapIsZoomed=!0,c.innerHTML="original size"),a.mapObject.checkResize(),window.scrollTo(0,0))},printDirections:function(){var b=document.getElementById("directionsInnerDiv");null!==b?a.openDirectionsPopup(b.innerHTML):a.updateStatus("Could not find directions.")},attachStyleDirections:function(b){var c=a.opts.styleSheetUrl;if(b.createStyleSheet)b.createStyleSheet(c);else{var d=document.createElement("link");d.rel="stylesheet";d.href=c;
b.getElementsByTagName("head")[0].appendChild(d)}},openDirectionsPopup:function(b){var c="";b?c='<html><head><title>Directions</title><style type="text/css"></style><body onLoad="self.focus()"><p><a href="#" onclick="self.print(); return false;">Print This page</a>, <a href="#" onclick="self.close(); return false;">Close Window</a></p>'+b+"</body></html>":a.updateStatus("Could not find directions.");(b=window.open("","directions","width=500,height=500,menubar=1,resizable=1,scrollbars=1").document)?
(b.open(),b.write(c),a.attachStyleDirections(b),b.close()):alert("Could not open directions in new window, please try again.")},getlastMarkerLonLat:function(){return this.lastMarker.getPoint()},showMarkerFromList:function(b){a.closeLastInfoWindow();-1<b&&(google.maps.event.trigger(this.gmarkers[b],"click"),b=jQuery("#"+a.mapDivName),0<b.length&&jQuery("html,body").animate({scrollTop:b.offset().top},1E3))},closeLastInfoWindow:function(){a.lastInfoWindow&&a.lastInfoWindow.close()},antipodeanPointer:function(b,
a){b=0<b?-180+b:180+b;a*=-1;this.updateStatus("Hole Drilled!");return new google.maps.LatLng(a,b,!1)},checkLatitude:function(b){return b&&0!==b&&-90<=b&&90>=b?b:!1},checkLongitude:function(b){return b&&0!==b&&-180<=b&&180>=b?b:!1},checkZoom:function(b){return 0<b&&50>b?b:0},distancePerPixel:function(b){var c=a.mapObject.fromLatLngToDivPixel(b),c=new google.maps.Point(c.x+10,c.y),c=a.mapObject.fromDivPixelToLatLng(c);return b.distanceFrom(c)/10},checkForHiddenMarkers:function(b){var a=[];if(1<this.gmarkers.length)for(var d=
this.obscuringPixelDiv(b),e,f=0;f<this.gmarkers.length;f++)this.gmarkers[f].markerName!=b.markerName&&!this.gmarkers[f].view&&"marker"==this.gmarkers[f].type&&this.gmarkers[f].getPosition().lat()>=b.getPosition().lat()&&(e=this.obscuringPixelDiv(this.gmarkers[f]),this.GBoundIntersection(d,e)&&a.push(f));return[]},GBoundIntersection:function(b,a){if(b[0]>=a[0]&&b[0]<=a[2]||b[2]>=a[0]&&b[2]<=a[2]||b[0]<=a[0]&&b[2]>=a[2])if(b[1]>=a[1]&&b[1]<=a[3]||b[3]>=a[1]&&b[3]<=a[3]||b[1]<=a[1]&&b[3]>=a[3])return!0;
return!1},obscuringPixelDiv:function(a){var b=a.getIcon(),d=a.position;a=d.lat()-b.anchor.x;var e=d.lng()-b.anchor.y,b=d.lat()+(b.size.width-b.anchor.x),d=d.lng();return[a,e,b,d]},updateServerDone:function(b){a.updateStatus(b);a.updateStatus("done")},findAddressForm:function(){return' <p class="findAddressHtml">  <input type="text" size="60" id="mapAddress" class="infoTabInputAddress" />'+a.opts.defaultAddressText+'  <input type="button" value="find address" id="infoTabSubmitAddress" class="submitButton" onclick="'+
a.variableName+".findAddress(document.getElementById('mapAddress').value); return false;\" /> </p>"},helpHtml:function(){var a="<ul><li><b>keyboard tricks:</b><ul><li>keys <i>up</i>, <i>down</i>, <i>left</i>, <i>right</i>: moves the map continuously while the key is pressed. Two keys can be pressed simultaneously in order to move diagonally.</li><li>keys <i>+</i>, <i>-</i>: zoom in and out respectively</li></ul></li><li><b>mouse tricks:</b><ul><li><i>click on a marker</i> to find out more about the location<li><i>click</i> anywhere else on the map to zoom-in on that particular point</li><li><i>click-and-hold-down</i> anywhere on the map to drag in any direction</li>",
