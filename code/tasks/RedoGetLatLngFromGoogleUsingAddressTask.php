...
class RedoGetLatLngFromGoogleUsingAddressTask extends BuildTask {
	...
	function run($request) {
		...
		if($this->verbose) {echo "<h1>============= Start =============</h1>";flush(); ob_flush();}
		if($this->verbose) {
			...
			echo "<h3>============= Redoing $count Objects =============</h3>";flush(); ob_flush();
		}
		for($i = 0; $i < 1000000; $i = $i + $this->step) {
			...
			foreach($objects as $object) {
				...
				$object->write();
				if($object->Longitude) {
					if($this->verbose) {DB::alteration_message(" --- found ".$object->FullAddress, "created");;flush(); ob_flush();}
				}
				else {
					if($this->verbose) {DB::alteration_message(" --- not found ".$object->Address, "deleted");;flush(); ob_flush();}
				}
			}
			if($this->verbose) {echo "<h1>============= END =============</h1>";flush(); ob_flush();}
		}
	}
}
